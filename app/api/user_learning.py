...
async def record_search(
    ...
    try:
        success = await asyncio.to_thread(record_artist_search, user_id, artist_name)
        return {
            "success": success,
...
async def get_learned_artists(
    ...
    try:
        learned_artists = await asyncio.to_thread(get_user_learned_artists, user_id, limit)
...
async def rate_artist(
    ...
    try:
        ...
        updated = await asyncio.to_thread(update_artist_rating, user_id, artist_name, rating)
        return {
            "success": True,
...
async def mark_artist_favorite(
    ...
    try:
        updated = await asyncio.to_thread(mark_artist_as_favorite, user_id, artist_name, is_favorite)
        return {
            "success": True,
...
async def get_preferred_genres(
    ...
    try:
        ...
        preferred_genres = await asyncio.to_thread(get_user_preferred_genres, user_id)
...
async def get_personalized_recommendations(
    ...
    try:
        ...
        recommendations = await asyncio.to_thread(get_recommendations_for_user, user_id, limit)
        
        # Get user's learned artists for context
        learned_artists = await asyncio.to_thread(get_user_learned_artists, user_id, limit=5)
...
async def get_user_learning_stats(
    ...
    try:
        ...
        learned_artists = (await session.exec(
