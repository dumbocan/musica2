...
async def _backfill_album_tracks(spotify_id: str, album_id: int, artist_id: int) -> int:
    ...
    try:
        ...
        logger.warning(
            "[maintenance] Spotify album tracks backfill failed for %s: %r",
            spotify_id,
            exc,
            exc_info=True,
        )
...
def _fetch_youtube_backfill_targets(
    limit: int,
    retry_failed: bool,
) -> list[tuple[Track, Artist, Album | None]]:
    with get_session() as session:
        linked_ids = set(
            session.exec(
        ...
        if len(targets) < limit:
            ...
            fav_artist_stmt = (
                ...
                .where(UserFavorite.target_type == FavoriteTargetType.ARTIST)
                .where(Track.spotify_id.is_not(None))
                .order_by(UserFavorite.created_at.desc(), Track.popularity.desc(), Track.id.asc())
                .limit(remaining * 3)
            )
            fav_artist_rows = session.exec(fav_artist_stmt).all()

            for track, artist, album in fav_artist_rows:
                if not track.spotify_id or not artist.spotify_id:
                    continue
                if track.spotify_id in linked_ids or track.spotify_id in failed_ids:
                    continue
                if track.spotify_id in already_have:
                    continue
                targets.append((track, artist, album))
                already_have.add(track.spotify_id)
                if len(targets) >= limit:
                    break
...
async def _run_youtube_backfill(limit: int, retry_failed: bool) -> None:
    ...
    for track, artist, album in targets:
        if maintenance_stop_requested():
            return
        try:
            await youtube_client._throttle()
            ...
            logger.warning(
                "[maintenance] YouTube search failed for %s - %s: %r",
                artist.name,
                track.name,
                exc,
            )
...
async def _run_images_backfill(limit_artists: int | None, limit_albums: int | None) -> None:
    if maintenance_stop_requested():
        return
    logger.info(
        "[images] backfill start (artists=%s, albums=%s)",
        limit_artists or "all",
        limit_albums or "all",
    )
...
async def _run_chart_backfill(
    ...
    if not dates_to_scan:
        logger.info(
            "[maintenance] chart backfill: no dates to scan for %s/%s",
            chart_source,
            chart_name,
        )
        return
    ...
    for chart_date in dates_to_scan:
        ...
        try:
            ...
            logger.warning(
                "[maintenance] chart backfill %s/%s %s failed: %s",
                chart_source,
                chart_name,
                chart_date,
                exc,
