...
from app.models.base import Artist, Album, Track, User
...
class DatabaseTypeMigrator:
    ...
    def generate_migration_sql(self, table_name: str, field_name: str) -> List[str]:
        ...
        sql_statements = [
            ...
            f"BEGIN;",
            f"-- First, backup any invalid JSON data",
            ...
            f"",
            f"-- Update NULL values for invalid JSON",
            f"UPDATE {table_name} SET {field_name} = NULL WHERE {field_name} IS NOT NULL AND",
            f"({field_name} NOT LIKE '{{%' AND {field_name} NOT LIKE '[%' AND {field_name} NOT LIKE '{{%});",
            f"",
            f"-- Alter column type to JSONB",
            f"ALTER TABLE {table_name} ALTER COLUMN {field_name} TYPE JSONB",
            f"USING CASE",
            f"  WHEN {field_name} LIKE '{%}' THEN NULL",  -- Simple string, not JSON
            f"  WHEN {field_name} LIKE '[%]' OR {field_name} LIKE '{{%}' THEN {field_name}::jsonb",  -- Valid JSON
            f"  ELSE NULL",  -- Invalid JSON, set to NULL
            f"END;",
            f"",
            f"COMMIT;"
        ]
